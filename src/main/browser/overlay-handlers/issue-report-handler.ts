...
export class IssueReportHandler implements OverlayHandler {
  onShow(webContents: WebContents): void {
    webContents.send(MainToRendererEventsForBrowserIPC.SHOW_OVERLAY_PANEL, { type: 'issue-report' });
    webContents.focus();
  }

  onHide(webContents: WebContents): void {
    webContents.send(MainToRendererEventsForBrowserIPC.HIDE_OVERLAY_PANEL, { type: 'issue-report' });
  }
}
