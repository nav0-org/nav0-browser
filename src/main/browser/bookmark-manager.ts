...
export abstract class BookmarkManager {
  ...
  public static async initListeners() {
    ...
    ipcMain.handle(
      ...
      async (
        ...
        return await BookmarkManager.addRecord(
          appWindowId,
          title,
          url,
          faviconUrl,
          type as 'reference' | 'queue'
        );
      }
    );
    ...
    ipcMain.handle(
      ...
      async (event, appWindowId: string, bookmarkId: string, type: string) => {
        return await BookmarkManager.updateBookmarkType(
          appWindowId,
          bookmarkId,
          type as 'reference' | 'queue'
        );
    ...
    ipcMain.handle(
      ...
      async (
        ...
        return await BookmarkManager.fetchBookmarksWithStats(
          appWindowId,
          type as 'reference' | 'queue',
          searchTerm,
          limit,
  ...
  public static async fetchRecords(
    ...
    const stmt = db.prepare(
      'SELECT * FROM bookmark WHERE (url LIKE ? OR title LIKE ?) ORDER BY createdDate DESC LIMIT ? OFFSET ? ; '
    );
    const records = stmt.all(
      ...
      limit,
      offset
    ) as Array<BookmarkRecord>;
    return records;
  }

  public static async fetchBookmarksWithStats(
    appWindowId: string,
    type: 'reference' | 'queue',
    searchTerm?: string,
    limit?: number,
    offset?: number
  ): Promise<Array<BookmarkWithStats>> {
    searchTerm = searchTerm || '';
    limit = limit || 50;
    offset = offset || 0;
    const db = BookmarkManager.getDb(appWindowId);
    ...
    const records = stmt.all(
      ...
      limit,
      offset
    ) as Array<BookmarkWithStats>;
    return records;
  }

  public static async getCountByType(
    appWindowId: string
  ): Promise<{ queue: number; reference: number }> {
    const db = BookmarkManager.getDb(appWindowId);
  ...
  public static async addRecord(
    ...
    const newlyCreatedRecord: BookmarkRecord = {
      id,
      title,
      url,
  ...
  public static async updateBookmarkType(
    appWindowId: string,
    bookmarkId: string,
    type: 'reference' | 'queue'
  ): Promise<BookmarkRecord | null> {
