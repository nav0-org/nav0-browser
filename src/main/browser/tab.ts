...
export class Tab {
  ...
  private initEventHandlers() {
    ...
    this.webContentsViewInstance.webContents.setWindowOpenHandler(({ url, disposition }) => {
      ...
      // gapi.iframes opens cross-frame RPC / widget helper frames via window.open
      // (e.g. Gmail contact hovercards). They're internal machinery, not user
      // navigations — denying here keeps them out of tabs and the popup budget.
      if (Tab.isGapiHelperPopup(url)) {
        return { action: 'deny' };
      }
  ...
  // gapi.iframes tags its cross-frame RPC / widget helper frames with usegapi=1
  // (e.g. Gmail contact hovercards). These are programmatically generated by the
  // gapi loader and never a destination a user would open themselves.
  private static isGapiHelperPopup(url: string): boolean {
    try {
      return new URL(url).searchParams.get('usegapi') === '1';
    } catch {
      return false;
    }
  }
