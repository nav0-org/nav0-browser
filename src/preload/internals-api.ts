...
export function init() {
  ...
  const isPrivate =
    process.argv.find((arg) => arg.startsWith('--is-private='))?.split('=')[1] === 'true' || false;
  ...
  contextBridge.exposeInMainWorld('BrowserAPI', {
    ...
    addBookmark: async (
      ...
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.ADD_BOOKMARK,
        appWindowId,
        title,
        url,
        faviconUrl,
    ...
    removeBookmark: async (appWindowId: string, bookmarkId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.REMOVE_BOOKMARK,
        appWindowId,
        bookmarkId
      );
    },
    removeAllBookmarks: async (appWindowId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.REMOVE_ALL_BOOKMARKS,
        appWindowId
      );
    },
    fetchBookmarks: async (
      ...
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.FETCH_BOOKMARK,
        appWindowId,
        searchTerm,
        limit,
        offset
    ...
    updateBookmarkType: async (appWindowId: string, bookmarkId: string, type: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.UPDATE_BOOKMARK_TYPE,
        appWindowId,
        bookmarkId,
        type
      );
    },
    fetchBookmarksWithStats: async (
      appWindowId: string,
      type: string,
      searchTerm: string,
      limit: number,
      ...
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.FETCH_BOOKMARKS_WITH_STATS,
        appWindowId,
        type,
        searchTerm,
        limit,
    ...
    removeDownload: async (appWindowId: string, downloadId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.REMOVE_DOWNLOAD,
        appWindowId,
        downloadId
      );
    },
    removeAllDownloads: async (appWindowId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.REMOVE_ALL_DOWNLOADS,
        appWindowId
      );
    },
    fetchDownloads: async (
      ...
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.FETCH_DOWNLOAD,
        appWindowId,
        searchTerm,
        limit,
        offset
    ...
    pauseDownload: async (downloadId: string, appWindowId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.PAUSE_DOWNLOAD,
        downloadId,
        appWindowId
      );
    },
    resumeDownload: async (downloadId: string, appWindowId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.RESUME_DOWNLOAD,
        downloadId,
        appWindowId
      );
    },
    cancelDownload: async (downloadId: string, appWindowId: string) => {
      return ipcRenderer.invoke(
        RendererToMainEventsForBrowserIPC.CANCEL_DOWNLOAD,
        downloadId,
        appWindowId
      );
    },
    ...
    onTabActivated: (callback: (tab: { id: string; url: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_ACTIVATED, (_event, tab) =>
        callback(tab)
      );
    },
    onTabClosed: (callback: (data: { id: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_CLOSED, (_event, data) =>
        callback(data)
      );
    },
    onTabTitleUpdated: (callback: (data: { id: string; title: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_TITLE_UPDATED, (_event, data) =>
        callback(data)
      );
    },
    onTabFaviconUpdated: (callback: (data: { id: string; faviconUrl: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_FAVICON_UPDATED, (_event, data) =>
        callback(data)
      );
    },
    onTabUrlUpdated: (
      ...
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_URL_UPDATED, (_event, data) =>
        callback(data)
      );
    },
    onNavigationFailed: (callback: (data: { id: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.NAVIGATION_FAILED, (_event, data) =>
        callback(data)
      );
    },
    onDownloadStarted: (
      ...
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.DOWNLOAD_STARTED, (_event, data) =>
        callback(data)
      );
    },
    onDownloadProgress: (
      ...
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.DOWNLOAD_PROGRESS, (_event, data) =>
        callback(data)
      );
    },
    onDownloadCompleted: (
      ...
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.DOWNLOAD_COMPLETED, (_event, data) =>
        callback(data)
      );
    },
    onDownloadPaused: (callback: (data: { downloadId: string; fileName: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.DOWNLOAD_PAUSED, (_event, data) =>
        callback(data)
      );
    },
    onDownloadResumed: (callback: (data: { downloadId: string; fileName: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.DOWNLOAD_RESUMED, (_event, data) =>
        callback(data)
      );
    },
    ...
    onReaderModeStateChanged: (callback: (data: { id: string; isActive: boolean }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.READER_MODE_STATE_CHANGED, (_event, data) =>
        callback(data)
      );
    },
    onFindInPageResult: (
      ...
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.FIND_IN_PAGE_RESULT, (_event, data) =>
        callback(data)
      );
    },
    onShowFindInPageBar: (callback: (data?: { searchText?: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.SHOW_FIND_IN_PAGE_BAR, (_event, data) =>
        callback(data)
      );
    },
    ...
    onShowPermissionStrip: (callback: (data: any) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.SHOW_PERMISSION_STRIP, (_event, data) =>
        callback(data)
      );
    },
    ...
    onShowOverlayPanel: (callback: (data: { type: string; data?: any }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.SHOW_OVERLAY_PANEL, (_event, data) =>
        callback(data)
      );
    },
    onHideOverlayPanel: (callback: (data: { type: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.HIDE_OVERLAY_PANEL, (_event, data) =>
        callback(data)
      );
    },
    ...
    onTabLoadingChanged: (callback: (data: { id: string; isLoading: boolean }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_LOADING_CHANGED, (_event, data) =>
        callback(data)
      );
    },
    onTabPinned: (callback: (data: { id: string }) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.TAB_PINNED, (_event, data) =>
        callback(data)
      );
    },
    ...
    onPermissionPromptShow: (callback: (data: any) => void) => {
      ipcRenderer.on(MainToRendererEventsForBrowserIPC.SHOW_PERMISSION_PROMPT, (_event, data) =>
        callback(data)
      );
    },
